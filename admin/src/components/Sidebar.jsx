...
const Sidebar = () => {
  ...
  return (
    <div className='min-h-screen border border-gray-200 bg-white overflow-y-scroll no-scrollbar'>
        {
            aToken && <ul className='text-[#515151] mt-5 overflow-y-scroll no-scrollbar'>
                <NavLink className={({isActive})=> `flex items-center gap-3 py-3.5 px-3 md:px-9 md:min-w-72 cursor-pointer ${isActive ? 'bg-[#F2F3FF] border-r-4 border-primary' : ''}`} to={'/admin-dashboard'}>
                    <img src={assets.home_icon} alt="" />
                    <p className='hidden md:block'>Dashboard</p>
                </NavLink>
                <NavLink className={({isActive})=> `flex items-center gap-3 py-3.5 px-3 md:px-9 md:min-w-72 cursor-pointer ${isActive ? 'bg-[#F2F3FF] border-r-4 border-primary' : ''}`} to={'/admin-all-appointment'}>
        ...
        {
            dToken && <ul className='text-[#515151] mt-5 overflow-y-scroll no-scrollbar'>
