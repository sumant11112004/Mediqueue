...
const{userData, setUserData, token, backendurl, loadUserProfileData} = useContext(AppContext)
  ...
  const updateUserProfileData = async()=>{
    try {
      ...
      //formData.append('address',userData.JSON.stringify(userData.address))
  ...
  return userData && (
    <div className='max-w-lg flex flex-col gap-2 text-sm'>
      ...
      <div>
        <p className='text-neutral-600 text-xl underline mt-3'>Contact Information </p>
        <div className='grid grid-cols-[1fr_3fr] gap-y-2.5 mt-3 text-neutral-800'>
         ...
         <p className='text-sm'>
              {isEdit ? <input type="text" onChange={(e) => setUserData(prev => ({ ...prev, address: { ...prev.address, line1: e.target.value } }))} value={userData.address.line1} name="" id="" /> : userData.address.line1} <br />
              {isEdit ? <input type="text" onChange={(e) => setUserData(prev => ({ ...prev, address: { ...prev.address, line2: e.target.value } }))} value={userData.address.line2} /> : userData.address.line2}
            </p>
            */}          
        </div>
      </div>
      <div>
        <p className='text-neutral-600 text-xl underline mt-3'>Basic Information</p>
        <div className='grid grid-cols-[1fr_3fr] gap-y-2.5 mt-3 text-neutral-800'>
