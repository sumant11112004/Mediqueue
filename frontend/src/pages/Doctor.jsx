...
useEffect(()=>{
  ...
  return (
    <div>
      <p className='text-gray-800'>Browse through the doctor Speciality.</p>
        ...
        <div className='w-full grid grid-cols-1 md:grid-cols-4 lg:grid-cols-5 gap-4'>
          {
            filterDoc.map((item,index)=>(
                <div onClick={()=>navigate(`/appointment/${item._id}`)} className='border border-blur rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-10px] transition-all duration-500  ' key={index}>
